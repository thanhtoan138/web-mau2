...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.ADD_CART_LIST_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.ADD_CART_LIST_ERROR,
...
const addCartListEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.ADD_CART_LIST),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Balotour/Cart/addCart_list";
...
export default addCartListEpic;
