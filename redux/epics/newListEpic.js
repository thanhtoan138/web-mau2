...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        catchError(error => {
            let data_error = {
                type: actionTypes.NEW_LIST_ERROR,
                payload: { connect: false }
            };
            return of(data_error);
        })
    );
const newListEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.NEW_LIST),
    concatMap(action => of('').pipe(
        map(res => {
            let keyword='';
            api_url = actionTypes.BASE_URL_API + "Balotour/News/list";
        })
    ).pipe(switchMap(act => ajaxControl(token))))
);


export default newListEpic;
