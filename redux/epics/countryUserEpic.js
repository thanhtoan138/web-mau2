...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.COUNTRY_USER_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.COUNTRY_USER_ERROR,
...
const countryUserEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.COUNTRY_USER),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Balotour/Info/countryUser";
