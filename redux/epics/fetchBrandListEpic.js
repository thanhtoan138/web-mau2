...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        catchError(error => {
            let data_error = {
                type: actionTypes.FETCH_BRAND_LIST_ERROR,
                payload: { connect: false }
            };
            return of(data_error);
        })
    );
const fetchBrandListEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.FETCH_BRAND_LIST),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Brand/brandList";
        })
    ).pipe(switchMap(act => ajaxControl(token))))
);


export default fetchBrandListEpic;
