...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.SETUP_ADDRESS_DETAIL_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.SETUP_ADDRESS_DETAIL_ERROR,
...
const setupAddressDetailEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.SETUP_ADDRESS_DETAIL),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Balotour/info/setupAddressDetail?id=" + action.id;
...
export default setupAddressDetailEpic;
