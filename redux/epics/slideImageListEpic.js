...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.SLIDE_IMAGE_LIST_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.SLIDE_IMAGE_LIST_ERROR,
                payload: { connect: false }
            };
            return of(data_error);
        })
    );
const slideImageListEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.SLIDE_IMAGE_LIST),
...
export default slideImageListEpic;
