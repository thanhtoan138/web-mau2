...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.LANGUAGE_DETAIL_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.LANGUAGE_DETAIL_ERROR,
...
const languageDetailEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.LANGUAGE_DETAIL),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Balotour/Info/getLanguageDetail";
...
export default languageDetailEpic;
