...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        map(response => ({
            type: actionTypes.ORDER_VIEW_DETAIL_SUCCESS,
            payload: response.response
        })),
        catchError(error => {
            let data_error = {
                type: actionTypes.ORDER_VIEW_DETAIL_ERROR,
...
const orderViewEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.ORDER_VIEW_DETAIL),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Balotour/Cart/order_view_detail?order_code="+action.order_code;
