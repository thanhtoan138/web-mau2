...
const ajaxControl = (user_token, action) =>
    ajax({
        ...
        catchError(error => {
            let data_error = {
                type: actionTypes.CITY_LIST_ERROR,
...
const cityListEpic = (action$, store) => action$.pipe(
    ofType(actionTypes.CITY_LIST),
    concatMap(action => of(actionTypes.GET_TOKEN()).pipe(
        map(res => {
            token = JSON.parse(res);
            api_url = actionTypes.BASE_URL_API + "Balotour/Country/list_city";
